...
The two colons after janitor tells R to pull from the Janitor package.
Helps for when we want to pull just one item from package
...
A good way to skip first rows or to make the first rows not the column
names.
...
There are some times when loading the whole library is needed instead of
::. If there are multiple sheets you have to specify which. Can specify
range of import by columns and rows.
...
Never use read.cvs, always use read\_csv
